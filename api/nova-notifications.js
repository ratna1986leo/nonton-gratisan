...
const SAWERIA_STREAM_KEY='a37eeea944ee57f64b34ea25c08d6832';
...
export default async function handler(req,res){if(req.method!=='POST')return res.status(405).json({error:'Method not allowed'});try{auth(req);const action=String(req.body?.action||'recent');if(action==='saweria'){let startDate=String(req.body?.startDate||'').trim();if(!/^\d{2}-\d{2}-\d{4}$/.test(startDate)){const parts=new Intl.DateTimeFormat('id-ID',{timeZone:'Asia/Jakarta',day:'2-digit',month:'2-digit',year:'numeric'}).formatToParts(new Date());const day=parts.find(x=>x.type==='day')?.value||'01';const month=parts.find(x=>x.type==='month')?.value||'01';const year=parts.find(x=>x.type==='year')?.value||String(new Date().getUTCFullYear());startDate=day+'-'+month+'-'+year;}const u='https://backend.saweria.co/widgets/milestone-progress?start_date='+encodeURIComponent(startDate);const upstream=await fetch(u,{headers:{accept:'application/json','stream-key':SAWERIA_STREAM_KEY},cache:'no-store'});const raw=await upstream.text();let data={};try{data=JSON.parse(raw)}catch{throw new Error('Saweria milestone tidak mengembalikan JSON');}if(!upstream.ok)throw new Error(data?.error||'Saweria milestone HTTP '+upstream.status);const progress=Number(data?.data?.progress ?? data?.progress ?? 0)||0;return res.status(200).json({ok:true,progress,startDate});}const [comments,catalog]=await Promise.all([getComments(req),getCatalog()]);const n=[];
...
n.push({id:'system-nova-center-v1',type:'system',icon:'📢',title:'NOVA Notification Center aktif',text:'Notifikasi mencakup film baru, komentar yang dibalas NOVA, dan request film yang sudah tersedia.',time:new Date().toISOString()});
const seen=new Set();const out=n.filter(x=>{if(seen.has(x.id))return false;seen.add(x.id);return true;}).slice(0,30);
return res.status(200).json({ok:true,notifications:out,counts:{total:out.length,replies:out.filter(x=>x.type==='reply').length,requests:out.filter(x=>x.type==='request').length,films:out.filter(x=>x.type==='film').length,system:out.filter(x=>x.type==='system').length}});
}catch(e){return res.status(e.status||500).json({error:e.message||'Notification error'});}}