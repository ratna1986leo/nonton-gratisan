...
async function askAI(message,items){
  ...
  const matches=searchItems(items,message);
  const s=stats(items);
  const context={stats:s,matches};
  ...
  const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+process.env.OPENAI_API_KEY},body:JSON.stringify({model:MODEL,input})});
